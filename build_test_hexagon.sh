...
export CC_hexagon_unknown_linux_musl=hexagon-linux-musl-clang
export CC_hexagon_unknown_none_elf=hexagon-unknown-none-elf-clang

# Updated to use the current hexagon linux toolchain
export PATH=${PATH}:/opt/clang+llvm-20.1.4-cross-hexagon-unknown-linux-musl/x86_64-linux-gnu/bin

export QEMU_LD_PREFIX=/opt/clang+llvm-20.1.4-cross-hexagon-unknown-linux-musl/x86_64-linux-gnu/target/hexagon-linux-musl/usr
# Per-test-case timeout: 300 seconds (5 minutes) for individual test executables
export CARGO_TARGET_HEXAGON_UNKNOWN_LINUX_MUSL_RUNNER="timeout 300s qemu-hexagon -L ${QEMU_LD_PREFIX}"
